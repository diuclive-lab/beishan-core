...
**补：Go-DSL 执行器请求路径已收口（2026-05-30，R1）**——`gods_executor.go` 的 `GoExecutor.Run`
（**同步**执行路径）此前无 recover，开发者提供的 `TransformFn`/`BeforeExecute`/`AfterExecute` panic
会带栈中断请求。现 `Run` 加 `defer RecoverWith`→panic 转失败 `WorkflowResult`；并行子步骤的
`Recover` 升级为 `RecoverWith`→panic 子步骤补记失败 `StepResult`（不再静默消失）。**仍刻意保留**：
该文件另 5 处 panic 在**构造期(boot)**——Go-DSL 工作流（开发者 Go 代码，非用户输入）引用未注册/
未配宿主的工具时启动即崩。这是**有意的 fail-fast 配置守卫**（boot 时大声暴露接线错误，胜过运行时
才发现）；代价是一个坏工作流会拖垮整个 daemon 启动。是否改为「log-and-skip 单个坏工作流、daemon
照常启动」是一个 fail-fast vs 优雅降级的取舍，留待按需决策（仅 2 个注册点：`main.go:283/288`）。
