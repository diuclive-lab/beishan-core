...
## 19. 浏览器/搜索工具的边界（2026-05-30 硬化后的遗留）

2026-05-30 把 `browser_navigate`/`web_search` 的「裸抓搜索引擎被反爬 + 静默返垃圾」止住了
（真实 Chrome UA、反爬检测→报错、`web_search` 0 结果→`Success:false`、SSRF/密钥检查、搜索引擎
URL→拒绝并指向 `web_search`）。仍存在的边界：

1. **可靠搜索依赖搜索 API key**：免费爬搜索引擎已基本失效（实测 DuckDuckGo `html` 端点回 HTTP 202
   反爬挑战、0 结果；Bing keyless 仅偶得 1 条）。**`web_search` 要稳定可靠须配 `TAVILY_API_KEY`**
   （或 Firecrawl/Brave）。现网 daemon 需在其启动环境补该 key（包装脚本含密钥、不由本仓库管，由开发者加）。
   无 key 时 `web_search` 不注册、`browser_navigate` 拒绝搜索 → 搜不了，但**报错明确**（胜过静默垃圾）。

2. **`browser_click` 仍脆弱 + 相对链接被 SSRF 拦**：`browser_click` 用「同行 `href=` 字符串匹配」找链接，
   真实 HTML 上基本失效（本就是已知短板）；且 `browser_navigate` 补了 `isSafeURL` 后，相对链接
   （无 host）会被判为「私有/内网」拦截。`browser_click` 实用性低，未投入修复——需要时应让它先把相对
   URL 拼回 `browserURL` 的 base 再 navigate。文本模式浏览器整体定位是「抓已知 URL 的正文」，不是
   交互式浏览；交互/JS 场景用 `web_render`（Playwright）。

3. **`web_fetch`/`web_extract` 被错误门禁挡住**：这俩抓任意 URL、**不需要搜索 key**，却和 `web_search`
   一样被 `checkWebAvailable`（Tavily/Firecrawl/Brave）门禁。即无搜索 key 时连"抓网页"都不可用。
   这道门禁对 fetch 是错的，2026-05-30 因聚焦搜索问题未一并改，留作后续（改为 `web_fetch` 总是注册）。