...
| 类型 | 能否做模块 | 例 |
|------|-----------|-----|
| 机械 / 确定性（代码层保证） | ✅ 好模块 | verify（`terminal_exec` 跑 verify.sh）、scan_large_files（`code_stats`）、capability_inventory |
...
模块可以**摆出候选**，但拍板的是开发者。所以可先做的第一个模块是 `verify`——
它就是 `scripts/verify.sh` 的运行时工作流外壳，机械、可信、高频复用。
