> ⚠️ 此文件已归档（2026-05-28）。
> 内容覆盖 2026-05-23 ~ 2026-05-24 的 S 轮次/M 方案/D 债务等工作记录。
> 2026-05-24 之后的开发工作请查看 `docs/devlog/`。
> 文件本身不删除，保留历史可追溯。
...
| 维度 | 检查项 | 反面教材 |
|------|--------|---------|
| 深度 | 模块本身功能完整吗？测试覆盖了吗？ | taxonomy 分类器有 10 个 flag 但无消费者 |
| 广度 | 模块和现有系统的连接点在哪里？ | metrics 定义了 11 个标准指标但零引用 |

**每新增一个包，必须同时回答**：谁调用它？谁被它调用？它的数据流向哪里？
