...
| 层 | 包 | 职责 | 可修改性 |
|---|---|---|---|
| L1 内核 | `kernel/` | 注册 + 路由 + 消息转发（首轮经 DeepSeek，preRoute 可绕过） | 冻结不改 |
...
"不再修改"不是"绝对不能改"，而是"改的门槛极高"。任何想动 `kernel/` 的改动，
必须先通过两道关，缺一不可：

1. **特许令级别的理由**：不是优化、不是美观、不是"顺手"。只有"防御进程崩溃"这一类
   危及整体可用性的硬问题才够格。问自己："我的修复有没有达到'特许令'的级别？"
2. **外部不可达证明**：必须先证明这个目标**无法在 kernel 之外达成**。绝大多数看似
   "要改内核"的需求，其实可以在 kernel 调用的下游函数里解决。

**先例（2026-05-29，kernel.go:246 回调 goroutine panic 兜底）：**
当时用户**已明确批准**修改 kernel——`go notify.Callback(...)` 是裸 goroutine，panic 会
掀翻进程，属"特许令"级别的可用性问题。但调查发现：该 goroutine 的执行体 100% 是
`notify.Callback`（唯一非测试调用点就是这行 `go`），于是把 `defer observatory.Recover`
放进 `notify.Callback` 内部，**等价兜底、零 kernel 改动**。`internal/notify` 不在冻结区。

这条先例确立的规则：**即便手握批准，也优先走 kernel 外的路径，并且我们做到了。**
这是一次经过批准的、为解决进程崩溃风险的防御性修复，它**不构成**未来向 `kernel/`
添加新功能的先例——它恰恰证明了"想改内核 → 先找内核外的解法"通常能成。
（详见 `docs/MERGE_DECISIONS.md` 决策 15。）
...
## 代码格式立场（gofmt 不全库强制——有意为之）

**`gofmt -l .` 会列出约 115 个文件，这是预期的、有意的，不是疏忽。**

后续梳理开发过程的人若看到这个数字，请先读完本节再下结论。

### 现象

本项目大量使用手写对齐的 `/* ... */` 块注释作为文档注释（约 76 个 `.go` 文件），
正文紧跟 `/*` 同行、续行用空格对齐缩进——对散文式的设计说明更易读。例：
...
Go 1.26.1 的 gofmt 会把它"规范化"成 Go 标准块注释形式：正文移到 `/*` 下一行、
续行改用 **tab** 缩进。此外 gofmt 还会重排部分 struct tag 对齐。两者叠加，
导致约 115 个文件在 `gofmt -l .` 下"漂移"。

### 决策：不全库 `gofmt -w`，不把 gofmt 加进 CI gate

- **拒绝重排全部 115 文件。** 理由：(1) gofmt 的块注释规范化会让这些刻意排版的散文
...
- 看到 `gofmt -l .` 输出 ~115 个文件：**正常，别"顺手修一下"。** 全量 `gofmt -w` 是一次
  破坏可读性 + 触碰冻结区 + 制造巨型 churn 的操作，需要专门决策，不是清理动作。
- 若某天确实要统一格式，那是一个独立的、需评估的决策（见 `docs/MERGE_DECISIONS.md` 决策 14），
  且必然牵涉"是否愿意为美观而修改 kernel/"——届时按内核冻结规则走批准流程。
...
```
right_flowers/
  ├── openhuman.yaml              ← 右花 A (personal_context)
  ├── hermes_agent.yaml         ← 右花 B (coding_agent)
  ├── openclaw.yaml             ← 右花 C (agent)
...
- `docs/archived/absorb_right_flower.yaml` — 右花吸收标准化工作流（六步：探→评→吸→测→补→记，已归档）
...
- 类型: Go 内核 + Electron 前端的块级知识管理应用
- 角色: L3 吸收参考源（参考设计，零代码复制）
...
**保持协议调用**：
- Lute markdown 引擎（`github.com/88250/lute`）已识别为可直接 `go get`，MulanPSL 2.0 无冲突，待后续集成

**未吸收（设计决策）**：
| 能力 | 原因 |
|------|------|
| SQLite FTS5 全文搜索 | 现有 L0+L1 检索已满足需求，不引入 SQLite 依赖 |
| AV 属性视图（数据库式表格视图） | 当前不需要 |
| Electron 前端 | beishan-core 有独立 Web UI |
| 事务系统（transaction.go） | 当前单步写入足够，需时再补 |
| 插件市场（bazaar/） | 不适用 |
| Datalog 查询引擎 | 过于复杂，当前图谱查询用 typed_links 替代 |
