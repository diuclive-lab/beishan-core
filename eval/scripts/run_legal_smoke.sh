...
run_test() {
  ...
  response=$(curl -s -X POST "$API_URL/api/chat" \
    ...
    -d "{\"sender\":\"user\",\"recipient\":\"$recipient\",\"type\":\"$expected_type\",\"payload\":$(echo "$prompt" | jq -Rs .)}" 2>&1 || true)
...
info ""
info "── 全链路测试 ──"
run_test "legal_full_chain_01" "审查这份合同：乙方月工资8000元，试用期6个月，试用期工资4000元。" "legal_review_plugin" "legal_review" "legal_review_plugin" 60
