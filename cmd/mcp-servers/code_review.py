#!/usr/bin/env python3
import json, sys
SKILL_DEF={"name":"code_review","description":"审查代码质量、安全漏洞、性能问题","tools":[{"name":"review_code","description":"审查代码质量、安全漏洞、性能问题","inputSchema":{"type":"object","properties":{"code": {"type": "string", "description": "code"}, "language": {"type": "string", "description": "language"}},"required":["code", "language"]}}]}
HANDLERS={}
def tool(f): HANDLERS[f.__name__]=f; return f

@tool
def review_code(args):
    code=args.get("code",""); language=args.get("language","")
    return f"# 代码审查\n\n## 分析结果\n根据代码审查技能分析输入。\n\n---\n*Generated*"
